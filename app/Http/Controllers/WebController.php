...
class WebController extends Controller
{
    ...
    function index()
    {
        ...
        $keywords = "";
        $genderText = "male";
        $imageContents = file_get_contents('https://www.speckson.com/public/assets/images/product/02.svg');
        if (!$imageContents) {
            return response()->json(['error' => 'Unable to download image'], 400);
        }
        $tempPath = storage_path('app/temp_specs.svg');
        ...
        Studio lighting, white background, ecommerce photography style. Front-facing pose. The eyeglasses must remain unchanged from the reference. Do not redesign or modify the frame. Keep frame proportions identical. Professional product photography, Natural skin tone, Soft shadows, and Sharp focus.";
        $response = Http::withToken($this->open_api_key)
            ->attach(
                'image',
                fopen($tempPath, 'r'),
                'specs.png'
